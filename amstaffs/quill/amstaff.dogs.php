            ...
            <div class="container py-5">
                <div class="row g-5">
                    <div class="col-lg-12 wow fadeInLeft" data-wow-delay="0.2s">
                        <div class="mb-1">   
                            <p class="mb-4">
                                SKYE, our AZTRAZAR-bred female from champion parents, accumulated 21 championship points in as many shows. To meet the aim of quality and not quantity, through a selective process, HECTOR, currently at Stud, was imported from Czech Republic from a line of 12 Champions and Grand-Champions out of a family tree of 14 parents.
                            </p>
                            <?php 
                                $sql_dogs_list = "SELECT dog_name, dog_information, id FROM quill_dogs WHERE status = 'Yes' AND dog_is = 'Knowledge Base' ORDER BY dog_name ASC"; 
                                
                                $result_dogs_list = $conn->query($sql_dogs_list);
                        
                                if ($result_dogs_list && $result_dogs_list->num_rows > 0) {
                                    while ($row = $result_dogs_list->fetch_assoc()) {
                                        
                                        // Sanitize data for display
                                        $dog_name = htmlspecialchars($row['dog_name']);
                                        $dog_info = htmlspecialchars($row['dog_information']);
                                        $dog_id = htmlspecialchars($row['id']);
                                        
                                        // Define the URL for the gallery link
                                        // Assuming your gallery page takes the dog's ID as a parameter, e.g., 'dog-gallery?id=X'
                                        $gallery_url = "dog-gallery?each={$dog_id}"; 
                            ?>
                                 
                            <h4 class="text-dark mb-4"><?php echo $dog_name; ?></h4>
                            <p class="mb-3">
                                <?php echo nl2br($dog_info); // Use nl2br to display line breaks saved in the database ?>
                            </p>
                        
                            <a class="btn btn-dark py-1 px-3 px-sm-4 me-2 mb-2" href="<?php echo $gallery_url; ?>"> 
                                <span>View Gallery - <?php echo $dog_name; ?></span> <i class="fas fa-chevron-circle-right"></i>
                            </a>        
                            <hr class="my-3"> 
                            ...
                            <?php
                                    ...
                                    $result_dogs_list->free();
                                } else {
                                    // Display if no dogs are visible on the site
                                    echo '<p class="text-info">There are no dogs currently visible on the site.</p>';
                                }
                            ?>
