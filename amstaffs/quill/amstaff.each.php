...
<html lang="en">
   ...
   <!-- Contact Start -->
    ...
    <?php if ($total_images > 0) : ?>
        ...
        <div id="<?php echo $pswp_gallery_id; ?>" class="pswp-gallery row px-1" data-pswp-uid="1">
            ...
            <?php foreach ($gallery_images as $index => $image):
                ...
                // Default to a safe size in case the file is missing
                $full_width = 1600; 
                $full_height = 1200; 

                // --- DYNAMICALLY GET IMAGE SIZE (FIXED) ---
                
                // Use DOCUMENT_ROOT for the most reliable absolute path
                $image_server_path = __DIR__ . '/../../../' . $image_full_path; 
                
                // Check if the file exists and retrieve its dimensions
                if (file_exists($image_server_path)) {
                    // Use @ to suppress potential PHP warnings if file is corrupt or unreadable
                    $size = @getimagesize($image_server_path); 
                    if ($size) {
                        $full_width = $size[0]; // Width
                        $full_height = $size[1]; // Height
                    }
                }
            ?>
                <div class="col-4 col-md-4 mb-2 pswp-gallery-item">
                    <a href="<?php echo $image_full_path; ?>" 
                       ...
                       class="d-block rounded overflow-hidden shadow gallery-thumbnail-hover thumbnail-container thumbnail-img" 
                       style="cursor: pointer;"
                       title="<?php echo $image_caption; ?>">
                        
                        <img class="img-fluid w-100 thumbnail-img" 
        ...
        <style>
/* 1. Ensure the container row allows its children to grow vertically */
/* This targets the 'row px-1' element wrapping your thumbnails */
.row {
    /* Ensures the row does not impose a fixed height based on surrounding elements */
    align-items: flex-start; 
}

/* 2. Increase Size for a Bolder Look */
.thumbnail-container { 
    /* The intended height increase */
    height: 200px; 
    
    /* Adding a default subtle shadow and rounding for a handsome base appearance */
    border-radius: 20px;
    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
}

/* 3. Enhanced Hover Effect for "The Most Handsome" */
.gallery-thumbnail-hover { 
    /* Maintain smooth transition for scale and now box-shadow */
    transition: all 0.3s ease-in-out; 
    position: relative;
    overflow: hidden;
}

.gallery-thumbnail-hover:hover { 
    /* Scale up more noticeably */
    transform: scale(1.1); 
    opacity: 1.0;
    /* Elevated shadow effect to make it pop */
    box-shadow: 0 12px 24px rgba(0, 0, 0, 0.3), 0 0 0 4px rgba(23, 162, 184, 0.7); 
    z-index: 10; /* Ensures the hovered image sits above neighbors */
}

/* 4. Image Fit (Crucial: Ensures the image always covers the container size) */
.thumbnail-img {
    height: 100%;
    width: 100%;
    object-fit: cover; 
...
/* 5. Active/Selected Border Enhancement */
.border-info { 
    border-color: #17a2b8 !important; 
    border-width: 3px !important; /* Make the active border more prominent */
    box-shadow: 0 0 0 4px #17a2b8 !important; /* Add a strong outline glow */
}
</style>
