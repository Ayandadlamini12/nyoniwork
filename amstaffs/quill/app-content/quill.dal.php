<?php
    ...
    // We now rely on the litter_id, not the image_id.
    if (!isset($_GET['id'])) {
    ...
    if (isset($_GET['delete']) && $_GET['delete'] === 'true') {
        ...
        if ($stmt_paths) {
            ...
            // =========================================================================
            // STEP 2: DATABASE DELETION (Delete ALL records for the litter_id)
            // =========================================================================
            
            // Proceed only if we have successfully handled all files
            if ($files_deleted_count === $files_to_delete) {
                
                $sql_delete_all = "DELETE FROM quill_litters_gallery WHERE litter_id = ?";
                $stmt_delete = mysqli_prepare($conn, $sql_delete_all);
                
                if ($stmt_delete) {
                    mysqli_stmt_bind_param($stmt_delete, "i", $litter_id);
                    
                    if (mysqli_stmt_execute($stmt_delete)) {
                        $success = true;
                        $_SESSION['metadata'] = "Litter Gallery, Successfully Deleted";
                    } else {
                        $error_message = "Database Error: Failed to delete records for Litter ID " . $litter_id . ".";
                    }
        ...
        // The confirmation now uses the litter_id parameter in the link.
        echo "<script>
        if (confirm('WARNING: Are you sure you want to delete **ALL** gallery images associated with this Litter? This will delete all files and records. This action is irreversible. Click OK to delete or Cancel to go back.')) { 
            window.location.href='litter-delete-all?id=$litter_id&delete=true'; 
        } else {
