<?php 
    ...
    if (isset($_POST['ThisUpdateListing']) && $_POST['ThisUpdateListing'] == 1) {
        ...
        $old_gallery_image = $_POST['old_gallery_image']; // Holds the OLD IMAGE NAME (e.g., image.jpg)

        $folder = "../assets/uploads/listing/";
    
        $new_image_name = $old_gallery_image;
        $update_image = false;
        ...
        if (isset($_FILES['gallery_image']) && $_FILES['gallery_image']['error'] == UPLOAD_ERR_OK) {
            ...
            $target_file = $folder . $unique_filename;
            
            if (move_uploaded_file($temp_name, $target_file)) {
                
                $new_image_name = $unique_filename;
                $update_image = true;
    
                // 2b. Delete the old file from the directory
                if (!empty($old_gallery_image) && file_exists($folder . $old_gallery_image)) {
                    if (unlink($folder . $old_gallery_image)) {
                        // Successfully deleted old file
                    } else {
                        // Log error but continue with DB update
                        error_log("Failed to delete old image: " . $folder . $old_gallery_image);
                    }
                }
            } else {
                // Failed to move the new file (e.g., permissions issue). Keep old image and log error.
                error_log("Failed to move uploaded file. Keeping old image.");
                // $update_image remains false, and $new_image_name remains $old_gallery_image
            }
        }
    
    
        // 3. Prepare the SQL statement based on whether an image was updated
        if ($update_image) {
            // Case 1: Image was updated (Need to update gallery_image column)
            $sql = "UPDATE quill_listing 
                    SET gallery_title = ?, 
                        gallery_description = ?, 
                        list_category = ?, 
                        gallery_image = ? 
            ...
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssssi", 
                    ...
                    $content_description, 
                    $list_category, 
                    $new_image_name, // The new filename
                    $content_id
                );
            }
    
        } else {
            // Case 2: No new image uploaded OR upload failed (Do NOT update gallery_image column)
            $sql = "UPDATE quill_listing 
                    SET gallery_title = ?, 
                        gallery_description = ?, 
                        list_category = ? 
        ...
        // 4. Check if statement preparation succeeded
        if ($stmt === false) {
            error_log("Failed to prepare statement: " . mysqli_error($conn));
            $_SESSION['metadata'] = "Error preparing update. Please try again.";
            header("Location: quill-photos");
        ...
        // 6. Check for execution success
        if ($result) {
            $_SESSION['metadata'] = "Info Updated Successfully!";
            header("Location: quill-photos");
            exit();
        } else {
            // Log the error and give a generic message to the user
            error_log("Failed to execute statement: " . mysqli_stmt_error($stmt));
            $_SESSION['metadata'] = "Error updating info. Please try again.";
            header("Location: quill-photos");
            exit();
        }
        ...
        // Fallback redirect (should be caught by exit() above, but good practice)
        header("Location: quill-photos");
        exit();
    }
    ...
    if (isset($_POST['deleteImageListing']) && $_POST['deleteImageListing'] == 1) {
        ...
        if ($content_value && file_exists($full_file_path)) {
            ...
            if (unlink($full_file_path)) {
                ...
                if (mysqli_stmt_execute($stmt_delete)) {
                    // Database deletion successful
                    $_SESSION['metadata'] = "Information Deleted Successfully!";
                    header("Location: quill-photos");
                    exit();
                } else {
                    // Database deletion failed
    ...
    if (isset($_POST['addNewImageListing']) && $_POST['addNewImageListing'] == 1) {
        ...
        if (move_uploaded_file($tempname, $folder)) {
            ...
            // Target the new 'quill_gallery' table and match the columns:
            // gallery_image, gallery_title, gallery_description
            $insert_sql = "INSERT INTO quill_listing (gallery_image, gallery_title, gallery_description, list_category) VALUES (?, ?, ?, ?)";
            ...
            if ($stmt === false) {
                ...
                // Bind parameters: 's' = string (for the three fields)
                // Binding: (1) image path, (2) title, (3) description
                mysqli_stmt_bind_param($stmt, "ssss", $image_path_for_db, $content_title, $content_description, $list_category);
    
                if (mysqli_stmt_execute($stmt)) {
                    // Database insert successful
                    $_SESSION['metadata'] = "Info Saved Successfully!";
                } else {
                    // Database insert failed
                    error_log("Failed to execute statement: " . mysqli_stmt_error($stmt));
                    $_SESSION['metadata'] = "Error saving info to database.";
                }
                mysqli_stmt_close($stmt);
            }
            ...
            // File upload failed
            $_SESSION['metadata'] = "Info Not Saved, Check permissions of the 'gallery' folder.";
        }
    ...
    // ADD NEW BLOG POST LOGIC
    // =========================================================================
    if (isset($_POST['addNewBlogPost']) && $_POST['addNewBlogPost'] == 1) {
        ...
        // Cast reading time and published status to integers
        $reading_time = (int)($_POST['blog_reading_time'] ?? 0); 
        // If the checkbox is checked, it will be '1', otherwise '0' (assuming form default)
        $is_published = isset($_POST['blog_is_published']) ? 1 : 0; 
    
        // 2. Prepare file upload variables
        $permitted_chars = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
        // Generate an 8-character random string for unique naming
        $output = substr(str_shuffle($permitted_chars), 0, 8); 
    
        // File input name is 'blog_featured_image' (Update this if your form uses a different name)
        $original_filename = basename($_FILES["blog_image"]["name"] ?? '');
        $tempname = $_FILES["blog_image"]["tmp_name"] ?? '';
        
        $image_path_for_db = null; // Store NULL if no image is uploaded
        $upload_success = true; // Tracks file upload status
    
        // Check if a file was provided and handle upload
        if (!empty($original_filename) && is_uploaded_file($tempname)) {
            $newname = 'blog_'.$output . '_' . $original_filename; // Prefix with 'blog'
            // Server-side path where the file will be saved.
            // NOTE: Ensure the folder '../assets/uploads/blogs/' exists and is writeable.
            $folder = "../assets/uploads/blog/" . $newname; 
    
            if (move_uploaded_file($tempname, $folder)) {
                // Path stored in DB is relative to the web root for display
                $image_path_for_db = $newname;
            } else {
                $upload_success = false;
                // Set error message for session
                $_SESSION['metadata'] = "Featured Image Not Saved, Check permissions of the 'blogs' upload folder."; 
            }
        }
    
        // --- 3. DATABASE INSERT (Using prepared statement) ---
        // Proceed only if file upload succeeded AND required text fields are present
        if ($upload_success && !empty($title) && !empty($content) && !empty($slug)) {
            
            // Target the 'quill_blog' table and its columns
            $insert_sql = "INSERT INTO quill_blog (blog_image, blog_title, blog_slug, blog_content, blog_author, blog_category, blog_reading_time,blog_is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    
            // Prepare the statement
            $stmt = mysqli_prepare($conn, $insert_sql);
    
            if ($stmt === false) {
                error_log("Failed to prepare blog statement: " . mysqli_error($conn));
                $_SESSION['metadata'] = "Error preparing database insert. Check log for details.";
            } else {
                // Bind parameters: 'sssssisi' = 6 strings, 2 integers
                // (image, title, slug, content, author, category, reading_time, is_published)
                mysqli_stmt_bind_param($stmt, "sssssisi", 
                    $image_path_for_db, $title, $slug, $content, $author, $category, $reading_time, $is_published
                );
    
                if (mysqli_stmt_execute($stmt)) {
                    // Database insert successful
                    $_SESSION['metadata'] = "Blog Post Saved Successfully!";
                } else {
                    // Database insert failed
                    error_log("Failed to execute blog statement: " . mysqli_stmt_error($stmt));
                    $_SESSION['metadata'] = "Error saving blog post to database: " . mysqli_stmt_error($stmt);
                }
                mysqli_stmt_close($stmt);
            }
        } elseif (!$upload_success) {
            ...
            // Handle case where required text fields are missing
            $_SESSION['metadata'] = "Error: Title, Slug, and Content are required for a new blog post.";
        }
    ...
    // =========================================================================
    if (isset($_POST['DeleteBlogPost']) && $_POST['DeleteBlogPost'] == 1) {
        
        $blog_id = (int)$_POST['blog_id'];
        // This variable is assumed to be passed from the hidden field in the delete modal/form
        // It should contain the filename of the image as stored in the database.
        $image_path_for_deletion = $_POST['blog_image_path'] ?? null; 
    
        // 1. Delete file from the file system (if one exists)
        if (!empty($image_path_for_deletion)) {
            // Construct the full server path to the file
            // NOTE: We assume the image is stored in '../assets/uploads/blogs/' based on the ADD logic
            $file_to_delete = "../assets/uploads/blog/" . $image_path_for_deletion; 
    
            // Check if the file exists and is not a directory before attempting unlink
            if (file_exists($file_to_delete) && !is_dir($file_to_delete)) {
                if (unlink($file_to_delete)) {
                    error_log("Blog featured image deleted successfully: " . $file_to_delete);
                } else {
                    // Log the error but continue to delete the DB record
                    error_log("Warning: Failed to delete image file from disk: " . $file_to_delete);
                }
        ...
        // 2. Delete database record using prepared statement
        $delete_sql = "DELETE FROM quill_blog WHERE id = ?";
        ...
        if ($stmt === false) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                $_SESSION['metadata'] = "Blog Post Deleted Successfully!";
            } else {
                error_log("Delete execute failed: " . mysqli_stmt_error($stmt));
                $_SESSION['metadata'] = "Error deleting blog post from database.";
            }
            mysqli_stmt_close($stmt);
        }
    ...
    // =========================================================================
    if (isset($_POST['UpdateBlogPost']) && $_POST['UpdateBlogPost'] == 1) {
        ...
        $reading_time = (int)($_POST['blog_reading_time'] ?? 0);
        // If the checkbox is checked, it will be '1', otherwise '0'
        $is_published = isset($_POST['blog_is_published']) ? 1 : 0; 
    
        // Current image path stored in a hidden field from the modal
        // Note: The path here should be just the filename (e.g., 'blog_asdf1234_image.jpg')
        $current_image_path = $_POST['current_image_path'] ?? null; 
        $image_path_for_db = $current_image_path; // Assume current path unless updated
    
        // 2. Handle new file upload (input name is 'blog_featured_image_new')
        $upload_success = true;
        // Check for a file input named 'blog_featured_image_new' (a common naming convention for update forms)
        $new_image_file = $_FILES["blog_image_new"] ?? null; 
        ...
        if ($new_image_file && !empty($new_image_file["name"]) && is_uploaded_file($new_image_file["tmp_name"])) {
            ...
            $output = substr(str_shuffle($permitted_chars), 0, 8);
            $original_filename = basename($new_image_file["name"]);
            $newname = 'blog_update_'.$output . '_' . $original_filename;
            
            // Server-side path where the file will be saved.
            // NOTE: Must match the directory used in the ADD logic: '../assets/uploads/blogs/'
            $folder = "../assets/uploads/blog/" . $newname; 
    
            if (move_uploaded_file($new_image_file["tmp_name"], $folder)) {
                
                // Set new path for DB (just the filename)
                $image_path_for_db = $newname;
    
                // Attempt to delete old image file if it existed and wasn't a placeholder
                if (!empty($current_image_path)) {
                    $old_file_path = "../assets/uploads/blog/" . $current_image_path; 
                    if (file_exists($old_file_path) && !is_dir($old_file_path)) {
                        if (unlink($old_file_path)) {
                            error_log("Old blog image deleted successfully: " . $old_file_path);
                        } else {
                            error_log("Warning: Failed to delete old image file from disk: " . $old_file_path);
                        }
                    }
                }
        ...
        if ($upload_success && !empty($title) && !empty($content) && !empty($slug)) {
            ...
            if ($stmt === false) {
                ...
                // Bind parameters: 'sssssiiii' = 6 strings, 3 integers (image, title, slug, content, author, category, reading_time, is_published, id)
                mysqli_stmt_bind_param($stmt, "sssssiiii", 
                    $image_path_for_db, $title, $slug, $content, $author, $category, $reading_time, $is_published, $blog_id
                );
    
                if (mysqli_stmt_execute($stmt)) {
                    $_SESSION['metadata'] = "Blog Post Updated Successfully!";
                } else {
                    error_log("Update execute failed: " . mysqli_stmt_error($stmt));
                    $_SESSION['metadata'] = "Error updating blog post in database: " . mysqli_stmt_error($stmt);
                }
                mysqli_stmt_close($stmt);
            }
        } elseif (!$upload_success) {
            // Error message already set during upload fail
             ...
             $_SESSION['metadata'] = "Error: Title, Slug, and Content are required for update.";
        }
    ...
    // ADD NEW FAQ LOGIC
    // =========================================================================
    if (isset($_POST['addNewFAQ']) && $_POST['addNewFAQ'] == 1) {
        
        // 1. Sanitize and validate input data
        $question = $_POST['faq_question'] ?? ''; 
        $answer = $_POST['faq_answer'] ?? ''; 
        
        // Cast display order and published status to integers
        $order = (int)($_POST['faq_order'] ?? 0); 
        // If the checkbox is checked, it will be '1', otherwise '0'
        $is_published = isset($_POST['faq_is_published']) ? 1 : 0; 
    
        // --- 2. DATABASE INSERT (Using prepared statement) ---
        if (!empty($question) && !empty($answer)) {
            
            // Target the 'quill_faqs' table and its columns
            $insert_sql = "INSERT INTO quill_faqs (faq_question, faq_answer, faq_order, faq_is_published) VALUES (?, ?, ?, ?)";
    
            // Prepare the statement
            $stmt = mysqli_prepare($conn, $insert_sql);
    
            if ($stmt === false) {
                error_log("Failed to prepare FAQ statement: " . mysqli_error($conn));
                $_SESSION['metadata'] = "Error preparing database insert. Check log for details.";
            } else {
                // Bind parameters: 'ssii' = 2 strings, 2 integers
                // (question, answer, order, is_published)
                ...
                if (mysqli_stmt_execute($stmt)) {
                    // Database insert successful
                    $_SESSION['metadata'] = "FAQ Added Successfully!";
                } else {
                    // Database insert failed
                    error_log("Failed to execute FAQ statement: " . mysqli_stmt_error($stmt));
                    $_SESSION['metadata'] = "Error saving FAQ to database: " . mysqli_stmt_error($stmt);
                }
                mysqli_stmt_close($stmt);
            }
        } else {
            // Handle case where required text fields are missing
            $_SESSION['metadata'] = "Error: Both Question and Answer are required for a new FAQ.";
        }
        
        // --- 3. REDIRECT ---
        // Assuming 'quill-faqs-page' is the correct page to redirect to
        header("Location: quill-photos"); 
        exit();
    }
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-2 mb-md-0">
                            <ul class="nav nav-pills flex-column mt-md-0 mt-1">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-carousel" data-toggle="pill" href="#account-vertical-carousel" aria-expanded="false">
                                        <i class="feather icon-play"></i>
                                        Home Carousel
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-about" data-toggle="pill" href="#account-vertical-about" aria-expanded="false">
                                        <i class="feather icon-info"></i>
                                        About Section
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-headers" data-toggle="pill" href="#account-vertical-headers" aria-expanded="false">
                                        <i class="feather icon-layout"></i>
                                        Section Headers
                                    </a>
                                </li> 
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-blog" data-toggle="pill" href="#account-vertical-blog" aria-expanded="false">
                                        <i class="feather icon-feather"></i>
                                        Blog
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-services" data-toggle="pill" href="#account-vertical-services" aria-expanded="false">
                                        <i class="feather icon-list"></i>
                                        Featured Cards
                                    </a>
                                </li> 
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-faqs" data-toggle="pill" href="#account-vertical-faqs" aria-expanded="false">
                                        <i class="feather icon-help-circle"></i>
                                        FAQs
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-policy1" data-toggle="pill" href="#account-vertical-policy1" aria-expanded="false">
                                        <i class="feather icon-shield"></i>
                                        Privacy Policy
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-terms" data-toggle="pill" href="#account-vertical-terms" aria-expanded="false">
                                        <i class="feather icon-camera"></i>
                                        Terms & Conditions
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-testimonials" data-toggle="pill" href="#account-vertical-testimonials" aria-expanded="false">
                                        <i class="feather icon-file-text"></i>
                                        Testimonials
                                    </a>
                                </li> 
                                <li class="nav-item">
                                    <a class="nav-link d-flex text-dark" id="account-pill-contact" data-toggle="pill" href="#account-vertical-contact" aria-expanded="false">
                                ...
                                <div class="card-content">
                                    <div class="card-body">
                                        <div class="tab-content">
                                            ...
                                            <!-- Listings --> 
                                            <?php include 'quill.listing.php' ?> 
                                            
                                            <!-- Terms --> 
                                            <?php include 'quill.terms.php' ?>

                                            <!-- FAQs --> 
                                            <?php include 'quill.faqs.php' ?>

                                            <!-- POLICY --> 
                                            <?php include 'quill.policy.php' ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
