<?php
    ...
    if (!isset($_GET['id']) || !isset($_GET['image_name'])) {
        $_SESSION['metadata'] = "Select Litter Image You Wanna Delete!!";
    ...
    if (isset($_GET['delete']) && $_GET['delete'] === 'true') {
        ...
        // --- 2. DATABASE DELETION ---
        if ($file_deleted) {
            // 💡 FIX 2: Use the correct table (assuming 'quill_litters_gallery')
            // 💡 FIX 3: Use prepared statements for security (prevents SQL injection)
            $sql_delete = "DELETE FROM quill_litters_gallery WHERE id = ?";
            ...
            if ($stmt) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    $_SESSION['metadata'] = "Litter Image Deleted Successfully!!";
                    header("Location: kb-litter");
                    exit;
                } else {
        ...
        if (confirm('Are you sure you want to delete the gallery image? Click OK to delete or Cancel to go back.')) {
            ...
            window.location.href='litter-one-all?id=$image_id&image_name=$image_name&delete=true'; 
        } else {
